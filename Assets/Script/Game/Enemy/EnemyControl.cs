...
namespace ActionGame
{
	public class EnemyControl : MonoBehaviour {
		...
		public Vector3 GenItemDropPos()
		{
			m_TempVec2 = Random.insideUnitCircle.normalized;
			m_TempVec2.x *= m_Controller.bounds.size.x;
			m_TempVec2.y *= m_Controller.bounds.size.z;

			m_TempVec3.x = m_TempVec2.x;
			m_TempVec3.z = m_TempVec2.y;
			m_TempVec3 += transform.position;
			m_TempVec3.y = 0.01f;
		...
		{
			GetComponent<HpHudControl>().SetHpHudActive(true);
		}

		public void OnDisable()
		{
			GetComponent<HpHudControl>().SetHpHudActive(false);
		}
	}
}